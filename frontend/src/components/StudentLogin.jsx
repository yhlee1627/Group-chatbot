...
function StudentLogin() {
  ...
  const handleLogin = async () => {
    if (!studentId || !password || !selectedClassId) {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/students?student_id=eq.${studentId}`,
        {
          headers: {
            apikey: import.meta.env.VITE_SUPABASE_ANON_KEY,
            Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
      ...
      if (student?.password === password && student?.class_id === selectedClassId) {
        localStorage.setItem("studentId", studentId);
        localStorage.setItem("classId", selectedClassId);
        localStorage.setItem("studentName", student.name || "학생");
        navigate("/student");
      } else {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} style={formStyle}>
      <h2 style={headerStyle}>학생 로그인</h2>
      ...
      <input type="text" value={studentId} onChange={(e) => setStudentId(e.target.value)} style={inputStyle} />
